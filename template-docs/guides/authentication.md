...
The Settings page lists API grants together with account security controls. Revoking one grant does not sign the browser out. It blocks existing access tokens through the D1 grant check and revokes every refresh token tied to the grant.
...
OTP is used for email verification after signup and password reset. The OTP is 6 digits, expires in 300 seconds, and allows 3 attempts. OTPs are stored hashed.
...
6. Set `SYSTEM_EMAIL` to an address on the onboarded domain before the first preparation.
...
4. Set `SYSTEM_EMAIL` to an address on the verified domain before the first preparation.
5. Open the admin System settings workspace and select the Email tab.
6. Choose Resend, enter the API key, and save.

The `from` address is always the D1 administrator email created during first preparation. If Resend rejects mail, fix the sender domain and initialize a fresh deployment with the correct `SYSTEM_EMAIL`.
...
3. **Email provider availability.** Verification, password reset, and other email actions return `EMAIL_PROVIDER_UNAVAILABLE` when no Provider is configured. The login page derives the same state and hides the forgot-password link. Account email changes are disabled.
...
The administrator identity, public support address, and email sender come from the unique D1 administrator account. The first preparation creates it from `SYSTEM_EMAIL`, generates a random password, and prints the credentials once. Later preparations do not overwrite the email or password. `BETTER_AUTH_SECRET` and `CONFIG_ENCRYPTION_KEY` are generated by `prepare-cloudflare`; users do not configure them.
...
**Using an unverified sender for the first administrator.** Set `SYSTEM_EMAIL` to an address on the verified sending domain before the first preparation. Later preparations never overwrite the D1 administrator email or password.
