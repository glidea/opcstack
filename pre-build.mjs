...
function parseSupportedLocalesConfig() {
	const sourcePath = 'src/web/lib/i18n/locales.ts'
	if (!existsSync(sourcePath)) {
		console.error(`Error: Missing locale config file ${sourcePath}`)
		process.exit(1)
	}

	const source = readFileSync(sourcePath, 'utf-8')
	const localeArrayMatch = source.match(/supportedLocales\s*=\s*\[([^\]]*)\]\s*as const/)
	if (!localeArrayMatch) {
		console.error('Error: supportedLocales not found in locales.ts')
		process.exit(1)
	}

	const localeMatches = [...localeArrayMatch[1].matchAll(/['"]([^'"]+)['"]/g)]
	const supportedLocales = [...new Set(localeMatches.map((match) => match[1]))]
	if (supportedLocales.length === 0) {
		console.error('Error: supportedLocales is empty in locales.ts')
		process.exit(1)
	}

	const defaultLocaleMatch = source.match(/defaultLocale\s*=\s*['"]([^'"]+)['"]/)
	if (!defaultLocaleMatch) {
		console.error('Error: defaultLocale not found in locales.ts')
		process.exit(1)
	}
...
function listLocalesFromMessageFiles() {
	const messageDir = 'src/web/lib/i18n/messages'
	if (!existsSync(messageDir)) {
		console.error(`Error: Missing messages directory ${messageDir}`)
		process.exit(1)
	}

	return readdirSync(messageDir, { withFileTypes: true })
		.filter((entry) => entry.isFile() && entry.name.endsWith('.json'))
		.map((entry) => entry.name.slice(0, -5))
		.sort((a, b) => a.localeCompare(b))
}

function listLocalesFromPublicDocs() {
	const docsDir = 'public-docs'
	if (!existsSync(docsDir)) {
		console.error(`Error: Missing docs directory ${docsDir}`)
		process.exit(1)
	}
...
function listDocSlugs(locale) {
	const localeDir = join('public-docs', locale)
	if (!existsSync(localeDir)) {
		console.error(`Error: Missing docs locale directory ${localeDir}`)
		process.exit(1)
	}

	const files = walkMarkdownFiles(localeDir)
	return files
		.map((filePath) => {
			const slug = relative(localeDir, filePath).replace(/\\/g, '/').replace(/\.md$/i, '')
			return slug
		})
		.sort((a, b) => a.localeCompare(b))
}
...
function walkMarkdownFiles(dirPath) {
	...
	for (const entry of entries) {
		...
		if (entry.isDirectory()) {
			const nested = walkMarkdownFiles(nextPath)
...
function ensureSameLocaleSet(label, expected, actual) {
	const expectedSorted = [...expected].sort((a, b) => a.localeCompare(b))
	const actualSorted = [...actual].sort((a, b) => a.localeCompare(b))

	const missing = expectedSorted.filter((locale) => !actualSorted.includes(locale))
	const extra = actualSorted.filter((locale) => !expectedSorted.includes(locale))
	if (missing.length === 0 && extra.length === 0) {
		return
	}

	console.error(`Error: LOCALE_SET_MISMATCH_${label}`)
	if (missing.length > 0) {
		console.error(`Missing in ${label}: ${missing.join(', ')}`)
	}
	if (extra.length > 0) {
		console.error(`Extra in ${label}: ${extra.join(', ')}`)
	}
	process.exit(1)
}

function ensureDocsSlugParity(locales) {
	if (locales.length === 0) {
		console.error('Error: No locales configured')
		process.exit(1)
	}

	const baseLocale = locales[0]
	const baseSlugs = listDocSlugs(baseLocale)

	for (const locale of locales) {
		const slugs = listDocSlugs(locale)
		const missing = baseSlugs.filter((slug) => !slugs.includes(slug))
		const extra = slugs.filter((slug) => !baseSlugs.includes(slug))

		if (missing.length === 0 && extra.length === 0) {
			continue
		}

		console.error(`Error: DOC_SLUG_PARITY_MISMATCH_${locale}`)
		if (missing.length > 0) {
			console.error(`Missing slugs in ${locale}: ${missing.join(', ')}`)
		}
		if (extra.length > 0) {
			console.error(`Extra slugs in ${locale}: ${extra.join(', ')}`)
		}
		process.exit(1)
	}
}
