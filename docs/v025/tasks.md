...
## TODO 清单
- [x] 1. 先增加设置 Tab、父子开关、字段展示和文案行为测试并确认失败
- [x] 2. General 仅保留文档可用性并补充明确说明；主题继续由 `DESIGN_SYSTEM` ENV 唯一决定
- [x] 3. Authentication 以开放注册为父开关，Turnstile 只暴露动态开关，OAuth 回调使用只读文本和复制操作
- [x] 4. Email、Credits 和 Affiliate 使用业务动作命名、紧凑宽度和展开式配置，不使用独立状态徽标
- [x] 5. Payment 只管理收款状态、平台连接和高级路由；国家使用可搜索选择器，密钥状态与操作对齐，Webhook URL 使用只读代码文本
- [x] 6. AI Routing 将 Provider 列表移出后使用策略预设，自定义时才展示权重；任务保留期拆成独立区域
- [x] 7. 同步中英文文案、公开文档和后台指南并完成真实浏览器验收
...
## TODO 清单
- [x] 1. 先增加远端商品列表、单平台关联、环境一致性和重复关联的契约与服务测试并确认失败
- [x] 2. 为 Dodo 和 Creem Provider 增加远端商品发现接口，并通过管理 API 暴露当前已配置平台的商品列表
- [x] 3. 收口 Payment Product Schema，只保存系统身份、Provider、环境、远端商品 ID 和本地权益，不保留多 Provider 列或手填内部 ID
- [x] 4. 重做支付商品抽屉：选择平台、选择远端商品、填写发放积分或订阅权益、确认关联
- [x] 5. 空状态提供刷新和远端平台入口；列表展示平台、环境、远端商品、价格、权益和状态
- [x] 6. 更新 Checkout、Webhook、文档和测试，删除旧手填商品 ID 及兼容逻辑
- [x] 7. 完成真实本地支付商品关联流程和完整回归
...
## TODO 清单
- [x] 1. 先增加 Provider 自动 ID、人类可读类型、模型标签、官方地址和路由预设测试并确认失败
- [x] 2. Provider 创建时由服务端生成 ID；管理 API 不再接受调用方提供的 ID
- [x] 3. Provider 类型选择显示用途与实现名称，不展示 `image_gemini` 等内部枚举
- [x] 4. 名称按选择结果提供默认值，模型改为回车生成的可删除标签
- [x] 5. 官方实现隐藏并使用内置 Base URL，兼容接口才展示地址；成本系数与启用状态提供明确影响说明
- [x] 6. AI Routing 提供均衡、稳定优先、速度优先、成本优先和自定义预设，并保持保存后立即生效
- [x] 7. 更新技术设计、Provider 文档和真实浏览器 E2E
...
- [x] 5. 验证用户设置页 OAuth 关联、密码与 API Grant，验证公开文档和用户侧关键页面
- [x] 6. 通过公开 HTTP 验证 OAuth 授权 Scope、撤销和配置保存后立即生效
- [x] 7. 将真实地址、命令和结果写入验收记录，确认没有阻塞项后完成任务
