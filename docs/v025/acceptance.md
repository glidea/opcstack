...
- `pnpm test:e2e:first-run`
	...
	- 首次准备、重复准备和本地 HTTP E2E 全部通过
	- 通过 1 个首次启动 HTTP 测试和 43 个本地 HTTP E2E；9 个不属于该轮条件的场景按显式条件跳过
...
- `pnpm test:e2e:first-run`
  ...
  - 通过 In App Browser 管理员流程、1 个首次运行 HTTP 测试、43 个本地 HTTP E2E 和 1 个真实 CLI OAuth E2E
...
- 本地完整验证
  ...
  - `pnpm test:e2e:first-run`：空库首次准备、重复准备、43 个本地 HTTP 场景和 CLI OAuth 流程通过；10 个不适用场景按条件跳过
- 配置来源与遗留审计
