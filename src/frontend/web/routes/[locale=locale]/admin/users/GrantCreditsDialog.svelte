			...
			<div class="space-y-3">
				<dl class="divide-y rounded-md bg-muted/50 px-3 text-sm">
					<div class="flex items-center justify-between gap-4 py-2.5">
						<dt class="text-muted-foreground">{$_('admin.users.grant.user')}</dt>
						<dd class="min-w-0 truncate text-right font-medium">{user.email}</dd>
					</div>
					<div class="flex items-center justify-between gap-4 py-2.5">
		...
		{:else}
			<form class="space-y-3" onsubmit={reviewGrant}>
				<div class="flex gap-3">
					<Field.Field class="w-36" data-invalid={amountError !== ''}>
						...
						/>
						{#if amountError !== ''}<Field.Error>{amountError}</Field.Error>{/if}
					</Field.Field>
					<Field.Field class="flex-1">
						...
						<Textarea
							...
							class="min-h-[4.5rem] resize-none"
						/>
					</Field.Field>
				</div>
				<Field.Field>
					...
					<ToggleGroup.Root type="single" bind:value={expiryOption} variant="outline" spacing={0} class="w-full">
						<ToggleGroup.Item value="never" class="flex-1 text-xs" aria-label={$_('admin.users.grant.never')}>{$_('admin.users.grant.never')}</ToggleGroup.Item>
						<ToggleGroup.Item value="week" class="flex-1 text-xs" aria-label={$_('admin.users.grant.oneWeek')}>{$_('admin.users.grant.oneWeek')}</ToggleGroup.Item>
						<ToggleGroup.Item value="month" class="flex-1 text-xs" aria-label={$_('admin.users.grant.oneMonth')}>{$_('admin.users.grant.oneMonth')}</ToggleGroup.Item>
						<ToggleGroup.Item value="custom" class="flex-1 text-xs" aria-label={$_('admin.users.grant.custom')}>{$_('admin.users.grant.custom')}</ToggleGroup.Item>
					</ToggleGroup.Root>
					{#if expiryOption === 'custom'}
						<Input id="grant-custom-expiry" type="datetime-local" bind:value={customExpiry} aria-invalid={expiryError !== ''} />
					{/if}
					{#if expiryError !== ''}<Field.Error>{expiryError}</Field.Error>{/if}
				</Field.Field>
				<Dialog.Footer>
