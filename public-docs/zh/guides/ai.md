...
在**后台 > Configuration > AI** 保存路由权重和任务保留期，并在下方管理 Provider。创建 Provider 时必须填写名称、Provider Type、至少一个模型、Base URL、价格系数、启用状态和 API Key。新建、编辑、删除成功后只更新目标行，不会重置路由表单草稿。版本过期时需要显式刷新。页面只显示 API Key 是否已配置。
