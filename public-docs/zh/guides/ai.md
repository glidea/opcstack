...
在**后台 > 系统设置 > AI 路由**选择均衡、稳定优先、速度优先、成本优先或自定义策略，并设置任务保留期。打开**AI 提供商**管理 Provider 实体，先选择用途与实现，再用标签添加模型名称。内部 ID 由 OPCStack 生成。Base URL 默认填入官方地址，也可以改为代理或兼容接口地址。新建、编辑、删除成功后只更新目标行。版本过期时需要显式刷新。页面只显示 API Key 是否已配置。
