...
async function verifyPaymentProductJourney(page: Page, concurrentPage: Page): Promise<void> {
	...
	const createButton = concurrentPage.getByRole('button', { name: 'Create product', exact: true }).first()
	await expect(createButton).toBeDisabled()
	await expect(concurrentPage.getByText('Configure a payment platform before creating products', { exact: true })).toBeVisible()
	await expect(concurrentPage.getByRole('link', { name: 'Platform settings', exact: true }).first()).toBeVisible()
	await expect(concurrentPage.locator('#payment-product-id')).toHaveCount(0)
	await expect(concurrentPage.locator('#provider-product-id')).toHaveCount(0)
}

async function verifyAIProviderJourney(page: Page, concurrentPage: Page): Promise<void> {
	await goToHydrated(page, '/en/admin/configuration/ai')
	await page.locator('#ai-routing-speed').click()
	await page.getByRole('button', { name: 'Save', exact: true }).click()
	await expect(page.getByText('Configuration saved', { exact: true })).toBeVisible()
	const speedConfig: { routing_error_weight: number; routing_latency_weight: number; routing_price_weight: number } = await page.evaluate(async (): Promise<{ routing_error_weight: number; routing_latency_weight: number; routing_price_weight: number }> => {
		const response: Response = await fetch('/api/admin/get_ai_config', {
			method: 'POST',
			headers: { 'content-type': 'application/json' },
			body: '{}'
		})
		return response.json() as Promise<{ routing_error_weight: number; routing_latency_weight: number; routing_price_weight: number }>
	})
	expect(speedConfig).toMatchObject({ routing_error_weight: 0.25, routing_latency_weight: 1, routing_price_weight: 0.1 })
	await page.locator('#ai-routing-balanced').click()
	await page.getByRole('button', { name: 'Save', exact: true }).click()
	await expect(page.getByText('Configuration saved', { exact: true })).toBeVisible()
	await page.locator('#ai-routing-custom').click()
	await expect(page.locator('#ai-routing-error')).toBeVisible()
	...
	await expect(providerSheet.locator('#ai-provider-id')).toHaveCount(0)
	await expect(providerSheet.locator('#ai-provider-type')).toContainText('Image generation / Google Gemini')
	await expect(providerSheet.locator('#ai-provider-name')).toHaveValue('Google Gemini image')
	await expect(providerSheet.locator('#ai-provider-base-url')).toHaveValue('https://generativelanguage.googleapis.com')
	await providerSheet.locator('#ai-provider-base-url').fill('https://gemini-proxy.example.com')
	await expect(providerSheet.locator('#ai-provider-base-url')).toHaveValue('https://gemini-proxy.example.com')
	await providerSheet.locator('#ai-provider-models').fill('browser-image-model')
	await providerSheet.locator('#ai-provider-models').press('Enter')
	await providerSheet.locator('#ai-provider-models').fill('browser-image-model-v2')
	await providerSheet.locator('#ai-provider-models').press('Enter')
	await expect(providerSheet.getByText('browser-image-model', { exact: true })).toBeVisible()
	await expect(providerSheet.getByText('browser-image-model-v2', { exact: true })).toBeVisible()
	await providerSheet.locator('#ai-provider-type').click()
	await concurrentPage.getByRole('option', { name: 'Image generation / OpenAI-compatible', exact: true }).click()
	await expect(providerSheet.locator('#ai-provider-name')).toHaveValue('OpenAI-compatible image')
	await expect(providerSheet.locator('#ai-provider-base-url')).toHaveValue('https://api.openai.com/v1')
	...
	const providerRow = concurrentPage.getByRole('row').filter({ hasText: 'browser-image-model-v2' })
	await expect(providerRow).toContainText('Image generation / OpenAI-compatible')
	await expect(providerRow).toContainText('browser-image-model')
	await expect(providerRow).toContainText('browser-image-model-v2')
	...
	await stalePage.getByRole('row').filter({ hasText: 'browser-image-model-v2' }).getByRole('button', { name: 'Edit', exact: true }).click()
	providerSheet = concurrentPage.getByRole('dialog')
	const staleProviderSheet = stalePage.getByRole('dialog')
	await providerSheet.locator('#ai-provider-name').fill('Browser compatible image disabled')
	await providerSheet.locator('#ai-provider-enabled').click()
	await staleProviderSheet.locator('#ai-provider-name').fill('Browser compatible image stale')
	await providerSheet.getByRole('button', { name: 'Save', exact: true }).click()
	await staleProviderSheet.getByRole('button', { name: 'Save', exact: true }).click()
	await expect(staleProviderSheet.locator('#ai-provider-name')).toHaveValue('Browser compatible image stale')
	...
	await expect(providerRow).toContainText('Browser compatible image disabled')
